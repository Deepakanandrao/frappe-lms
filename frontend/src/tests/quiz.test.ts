...
describe('Quiz — proctoring violation handling', () => {
	beforeEach(() => {
		quizFixture = makeQuiz({ enable_proctoring: true, max_violations: 3 })
	})

	// Satisfies the camera gate and clicks Start so activeQuestion > 0.
	async function startProctoredQuiz() {
		const wrapper = mountQuiz()
		await flushPromises()
		const setupMonitor = wrapper
			.findAllComponents({ name: 'ProctoringMonitor' })
	...
	it('shows a warning toast for each violation below the limit', async () => {
		const wrapper = await startProctoredQuiz()
		// After start the status-bar monitor is the active one
		const monitor = wrapper
			.findAllComponents({ name: 'ProctoringMonitor' })
			.find((m) => m.props('active') === true)!
		await monitor.vm.$emit('violation', 'tab_switch')
		expect(_toast.warning).toHaveBeenCalledTimes(1)
		await monitor.vm.$emit('violation', 'no_face')
		expect(_toast.warning).toHaveBeenCalledTimes(2)
	})

	it('auto-submits with submission_reason="max_violations" when limit is reached', async () => {
		const wrapper = await startProctoredQuiz()
		const monitor = wrapper
			.findAllComponents({ name: 'ProctoringMonitor' })
			.find((m) => m.props('active') === true)!
		await monitor.vm.$emit('violation', 'tab_switch')
		await monitor.vm.$emit('violation', 'no_face')
		await monitor.vm.$emit('violation', 'tab_switch') // 3rd = max
		// submitQuiz has a 500 ms setTimeout before calling createSubmission
		vi.runAllTimers()
		expect(_submitSpy).toHaveBeenCalledWith(
			expect.objectContaining({ submission_reason: 'max_violations' }),
			expect.anything()
		)
	})

	it('does not submit or show toast for warning events', async () => {
		const wrapper = await startProctoredQuiz()
		const monitor = wrapper
			.findAllComponents({ name: 'ProctoringMonitor' })
			.find((m) => m.props('active') === true)!
		await monitor.vm.$emit('warning', 'no_face')
		await monitor.vm.$emit('warning', 'no_face')
		vi.runAllTimers()
		expect(_submitSpy).not.toHaveBeenCalled()
		expect(_toast.warning).not.toHaveBeenCalled()
	})

	it('ignores violations once submission is already in progress', async () => {
		const wrapper = await startProctoredQuiz()
		const monitor = wrapper
			.findAllComponents({ name: 'ProctoringMonitor' })
			.find((m) => m.props('active') === true)!
		// Trigger max_violations to start the submit flow
		await monitor.vm.$emit('violation', 'tab_switch')
		await monitor.vm.$emit('violation', 'tab_switch')
		await monitor.vm.$emit('violation', 'tab_switch')
		vi.runAllTimers()
		const callsBefore = _submitSpy.mock.calls.length
		// A 4th violation must be silently dropped
		await monitor.vm.$emit('violation', 'tab_switch')
		vi.runAllTimers()
		expect(_submitSpy.mock.calls.length).toBe(callsBefore)
	})
})
